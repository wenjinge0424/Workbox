...
- (void) launchPageViewController {
    ...
    pageTitleLabels = @[LOCALIZATION(@"welcome1"), LOCALIZATION(@"welcome2"), LOCALIZATION(@"welcome3")];
...
- (void) gotoLoginScreen {
    ...
    if (nType == ACCOUNT_TYPE_WORKER) {
        [self performSegueWithIdentifier:@"showWorkerHomeVC" sender:Nil];
    }
    else if (nType == ACCOUNT_TYPE_EMPLOYER) {
        [self performSegueWithIdentifier:@"showEmployerHomeVC" sender:Nil];
    }
    else {
        [self performSegueWithIdentifier:@"showAdminHomeVC" sender:Nil];
    }
...
- (PageItemViewController *)viewControllerAtIndex:(NSUInteger)index
{
    ...
    pageContentViewController.welcomeText = pageTitleLabels[index];
